...
class TemporalContextBlock(nn.Module):
    ...
    def process_each(self, feature_maps, position): # postion 0-4 which postion is main
        ...
        for i in range(S):
            idx = i+1+addition
            attbt_name = 'tconv_2_{}'.format(idx)
            attbt_i = getattr(self, attbt_name)
            step2 =  attbt_i(feature_maps[:,i,...]) # [B,C,H,W]
            
            # [B, C, H, W] -> [B,C,H*W]
            step_2_final = step2.view(B,C,-1) 
            step2s.append(step_2_final)
        ...
        for i in range(S):
            ...
            # [B, 1, H, W] -> [B,1,H*W]
            step1_final = step1.view(B,1,-1)
            step1s.append(step1_final)
        ...
        if self.local_mean:
            # Need to understando more about this local mean operation. Why multiply by the mean? Why permute?
    ...
    def process_each_all(self, feature_maps, position): # only in mode2 and mode3
        ...
        if self.mode =='mode2':
            ...
            # step1s: [B,S,1,H,W] = seprate_conv_stack(feature_maps: [B,S-1,C,H,W], main: [B,C,H,W])
        ...
        # step1s: [B,S,1,H,W] -> [B,S,1,H*W]
        # step2s: [B,S,C,H,W] -> [B,S,C,H*W]
        step1s = step1s.view(B,S,1,H*W)
        step2s = step2s.view(B,S,C,H*W) 
        
        step1s=self.temporal_softmax(step1s) 
        ...
        if self.mode=='mode2':
            original_features = torch.cat([feature_maps, main.unsqueeze(1)],1) 
            feature_maps = original_features + step2s.unsqueeze(-1) #[B,S,C,H,W]
        ...
        if self.mode=='mode2':
            ...
            feature_selection = feature_maps[:,:,:-1,...] # [B,C,S-1,H,W]
            feature_permuted = feature_selection.permute(0,2,1,3,4).contiguous() # [B,S-1,C,H,W]
            feature_reshaped = feature_permuted.view(-1,C,H,W) # [B*(S-1),C,H,W]

            output = self.tconv_3_o(feature_reshaped)  
    ...
    def forward(self,x,snip):
        ...
        if self.mode=='mode1':
            ...
            if not self.reduce:
                for i in range(snip_size):
                    feature_maps=x
                    # Process each frame as the main frame, and the rest frames as context frames. The position of the main frame is determined by i.
